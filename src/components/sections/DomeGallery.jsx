  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (originalImg) {
        ...
        img.style.cssText = 'width: 100%; height: 100%; object-fit: contain; background: #0a0a0b;';
  ...
  const openItemFromElement = el => {
    ...
    if (rawTitle || rawDescription) {
      ...
      caption.style.cssText = `
        ...
        bottom: 0;
        padding: 18px 22px;
        background: linear-gradient(to top, rgba(9,9,11,0.9) 0%, rgba(9,9,11,0.6) 60%, rgba(9,9,11,0) 100%);
        color: #f4f4f5;
        pointer-events: none;
        opacity: 0;
        transform: translateY(8px);
        transition: opacity 320ms ease ${enlargeTransitionMs}ms, transform 320ms ease ${enlargeTransitionMs}ms;
      `;
      if (rawTitle) {
        const t = document.createElement('div');
        t.textContent = rawTitle;
        t.style.cssText = 'font-size: 18px; font-weight: 600; letter-spacing: -0.01em; margin-bottom: 4px;';
        caption.appendChild(t);
      }
      if (rawDescription) {
        const d = document.createElement('div');
        d.textContent = rawDescription;
        d.style.cssText = 'font-size: 13px; line-height: 1.5; color: #a1a1aa; max-width: 92%;';
        caption.appendChild(d);
      }
      overlay.appendChild(caption);
      // fade in after the overlay scale animation
      setTimeout(() => {
